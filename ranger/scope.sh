...
## Because ofb some automated testing we do on the script #'s for comments need
...
handle_image() {
    ...
    case "${mimetype}" in
        ...
        ## DjVu
        image/vnd.djvu)
            ddjvu -format=tiff -quality=90 -page=1 -size="1920x1080" \
        ...
        ## Other cases (uncomment and modify as needed)
        # video/*)
        ...
        #     epub-thumbnailer "${FILE_PATH}" "${IMAGE_CACHE_PATH}" "1920" && exit 6
        #     exit 1;;
        
        # application/font*|application/*opentype)
        #     preview_png="/tmp/$(basename "${IMAGE_CACHE_PATH%.*}").png"
        #     if fontimage -o "${preview_png}" --pixelsize "120" --fontname --pixelsize "80" --text "Sample Text" "${FILE_PATH}"; then
